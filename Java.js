var allRules = [
    { hostname: ["agar.io"],
      ...
      replace: function (m) {
          ...
          !vAlive && console.error("RSBotz loader failed to find value1")
          !vEaten && console.error("RSBotz loader Failed to find value2")
          if (vAlive && vEaten)
...
if (window.top != window.self)
    ...
    return console.error("RSBotz loader Please try to reload the page, or make sure this script runs at document-start so we can inject the new code before the old code runs.")
...
if (!rules)
    return console.error("RSBotz loader Did you modify the code? Please dont...")
...
function tryReplace(node, event) {
    ...
    var mod = {
        ...
        replace(what, from, to, reset) {
            ...
            if(newText === this.text) {
                console.error("RSBotz loader errored: `" + what + "` ... Injection failed!")
                return false
            } else {
                this.text = newText
                if (reset)
    ...
    if (scriptEmbedded) {
        ...
        console.log("RSBotz loader injected 2nd set of code into page")
    } else {
        document.head.removeChild(node)
        ...
        request.onload = function() {
            ...
            function insertScript(script) {
                if (typeof jQuery === "undefined")
                    ...
                    document.head.appendChild(script)
                    console.log("RSBotz loader injected 1st set of code code into page.")
            }
            insertScript(script)
        }
        request.onerror = function() { console.error("RSBotz loader failed to inject code!") }
